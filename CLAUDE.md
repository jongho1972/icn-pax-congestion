...
| `templates/index.html` | Jinja2 템플릿 — 비번 게이트 + **핵심 요약 카드(SMS 동일 기준 + 환율 + ▲▼ MTD 대비)** + 일자별 섹션(T1·T2 일자별 추이 라인 2개 + 시간대별 라인 2개 + D-29~D+1 표) + 월누적 섹션(요약+표) + 출국장 평면도 SVG 2개 + 도착지별(권역) 그룹바 2개 |
...
화면 구조는 항공편수 대시보드와 동일한 패턴(일자별 → 월누적 → 출국장별 → 도착지별)을 따른다.
...
- **시간대별 차트 / 출국장별 평면도 / 도착지별 권역**은 엑셀 `2.출국장별 예상여객`/`3.시간대별 출국장별`/`출국노선별` 시트가 raw 데이터로만 제공돼 **출국장 통과 기준**(환승객 제외) 유지. `hourly_t1_t2`·`hourly_per_gate`·`gate_compare`·`route_compare`가 해당.
- 누적 계열(월누적·출국장별·도착지별)은 모두 이번달 1~D-1 vs 전월 1~D-1 동일일수 비교.
...
2. **일자별** 섹션:
   ...
   - 시간대별 차트 2개 (T1·T2 분리, MTD 평균 점선 + focus 일자 실선, y축 동일)
